...
需要云端工具时，可在设置中配置自己的 API Key。当前提供以下可选实验集成，真实服务验收仍待补充：

| 工具 | 用途 | 发送到服务商的内容 |
|---|---|---|
| Soniox stt-rt-v5 | 云端实时语音识别 | 主动开始、导入或重试的对应音频，以及配置的词表提示。 |
...
选择 Soniox 后，原始录音仍保存在本机。“暂停云端转写”会停止发送后续音频，本地录音继续保存；恢复连接后可主动重试。通过 Soniox 导入的 WAV 按正常播放速度发送，处理时长与录音长度相关。
...
0.1.2 适合先用短课程记录体验完整流程。Windows 打包与实机运行、系统声音采集、云端服务和 120 分钟连续录音的验收待完成。实际识别速度与效果取决于电脑配置、音频质量和所选服务。
