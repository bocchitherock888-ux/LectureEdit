...
export const demoAdapter: LectureAdapter = {
  ...
  importAudio: async () => { throw new Error('浏览器演示只展示交互。请在 LectureEdit 桌面应用中导入 WAV。'); },
  importPackage: async () => { throw new Error('浏览器演示只展示交互。请在 LectureEdit 桌面应用中导入课程包。'); },
  exportSession: async () => { throw new Error('浏览器演示不会写入文件。请在 LectureEdit 桌面应用中导出。'); },
