...
export default function App() {
  ...
  const [engineOpen, setEngineOpen] = useState(false); const [pendingEngine, setPendingEngine] = useState<'qwen' | CloudEngine | undefined>(undefined);
  ...
  const cloudEngine = isCloudEngine(state?.settings.engine);
  const activeCloud = cloudProvider(state?.settings.engine);
  ...
  const cloudKeySaved = (engine: CloudEngine) => Boolean(runtime.cloudKeys?.[engine] ?? (engine === 'soniox' && runtime.cloudKeyConfigured));
  const selectEngine = async (engine: 'qwen' | CloudEngine) => {
    if (!state || engineSwitchBlocked) return;
    setEngineOpen(false);
    if (engine === state.settings.engine) return;
    if (isCloudEngine(engine) && (!cloudKeySaved(engine) || !state.settings.cloudConsent)) { setPendingEngine(engine); setModal('settings'); return; }
    ...
    <GlassSurface className="workspace">
      <header className="topbar" aria-label="课程工具栏">
        ...
        <div className="toolbar-group model-tools" aria-label="转写设置">
          ...
          <div className="menu-wrap" ref={engineWrapRef}><button ref={engineButtonRef} className="engine-button" aria-label="选择转写引擎" aria-haspopup="menu" aria-expanded={engineOpen} disabled={engineSwitchBlocked} title={captureInProgress ? '结束录音后切换转写引擎' : ['downloading', 'verifying'].includes(runtime.modelDownload?.phase ?? 'idle') ? '模型准备完成后切换转写引擎' : '选择转写引擎'} onClick={() => { setEngineOpen((open) => !open); setExportOpen(false); setMoreOpen(false); }} onKeyDown={(event) => { if (event.key === 'ArrowDown') { event.preventDefault(); setEngineOpen(true); setExportOpen(false); setMoreOpen(false); requestAnimationFrame(() => engineMenuRef.current?.querySelector<HTMLElement>('[role="menuitem"]')?.focus()); } }}>{activeCloud ? <ProviderMark engine={activeCloud.engine} size={16} /> : <HardDrive size={14} />}<span>{activeCloud ? activeCloud.name : legacyEngine ? '选择引擎' : 'Qwen'}</span><ChevronDown className={`control-chevron ${engineOpen ? 'is-open' : ''}`} size={13} strokeWidth={1.9} /></button>{engineOpen && <div ref={engineMenuRef} className="export-menu engine-menu" role="menu" onKeyDown={moveMenuFocus}><button role="menuitem" className={!cloudEngine && !legacyEngine ? 'selected' : ''} onClick={() => void selectEngine('qwen')}><span className="engine-local-mark" aria-hidden="true"><HardDrive size={13} /></span><span><strong>Qwen3-ASR</strong><small>在这台电脑上识别</small></span></button>{CLOUD_PROVIDERS.map((item) => <button key={item.engine} role="menuitem" className={state.settings.engine === item.engine ? 'selected' : ''} onClick={() => void selectEngine(item.engine)}><ProviderMark engine={item.engine} size={22} /><span><strong>{item.name}</strong><small>{cloudKeySaved(item.engine) ? `云端 · ${item.price}` : '云端 · 需要填写 API Key'}</small></span></button>)}</div>}</div>
